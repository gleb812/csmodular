...
export class CsoundGenerator {
    constructor() {
        ...
        // ⭐ Счётчики шин для zakinit
        this._audioBusCount = 2;   // по умолчанию: 0 и 1 служебные
        this._controlBusCount = 2; // по умолчанию: 0 и 1 служебные
        
        // ⭐ Активные кабели (для отслеживания)
        this._activeCables = [];
        
        // Шаблон Csound (будет обновляться динамически)
...
// src/csound/CsoundGenerator.js
     ...
     */
    addCable(cable) {
        this._activeCables.push(cable);
        console.log(`🔌 Cable added to _activeCables. Total: ${this._activeCables.length}`);
        console.log('  Cable color:', cable.getDisplayColor ? cable.getDisplayColor() : cable.color);
        this._updateZakInit();
        this.log(`🔌 Added cable, zakinit updated`);
    }

    /**
     * Обновить zakinit на основе кабелей
     */
    _updateZakInit() {
        console.log(`🔍 _updateZakInit called. Active cables: ${this._activeCables.length}`);
        
        let audioCount = 0;
        let controlCount = 0;
        
        for (const cable of this._activeCables) {
            const cableType = this._getCableType(cable);
            if (cableType === 'audio') {
                audioCount++;
            } else if (cableType === 'control') {
                controlCount++;
            }
        }
        
        console.log(`  audioCount: ${audioCount}, controlCount: ${controlCount}`);
        
        this._audioBusCount = 2 + audioCount;
        this._controlBusCount = 2 + controlCount;
        
        console.log(`🎛️ zakinit: audio=${this._audioBusCount}, control=${this._controlBusCount}`);
    }

    /**
     * Получить строку zakinit
     */
    getZakInitString() {
     ...
     */
    removeCable(cable) {
        const index = this._activeCables.indexOf(cable);
        if (index > -1) {
            this._activeCables.splice(index, 1);
            this._updateZakInit();
            this.log(`🔌 Removed cable, zakinit updated`);
        }
    }
    ...
    clearCables() {
        this._activeCables = [];
        this._updateZakInit();
        this.log(`🔌 All cables cleared, zakinit reset`);
    }
     ...
     */
    _updateZakInit() {
        // Просто считаем количество кабелей каждого типа
        let audioCount = 0;
        let controlCount = 0;
        ...
        // ⭐ audioBuses = 2 (служебные: 0 и 1) + количество audio кабелей
        this._audioBusCount = 2 + audioCount;
        // ⭐ controlBuses = 2 (служебные: 0 и 1) + количество control кабелей
        this._controlBusCount = 2 + controlCount;
    ...
    /**
     * Получить следующий индекс для шины
     */
    _getNextBusIndex(cable, type) {
        const cablesOfType = this._activeCables.filter(c => {
            return this._getCableType(c) === type;
        });
        
        // Индекс = порядковый номер кабеля данного типа + 1 (так как 0 и 1 служебные)
        const index = cablesOfType.indexOf(cable);
        return index + 1;
    }
    ...
    addModule(moduleData) {
        ...
        console.log('  ✅ Module saved with defaultMode:', defaultMode);
    ...
    generateFullCsd() {
        ...
        // 4. ⭐ Формируем zakinit строку
        const zakinitString = this.getZakInitString();
        
        // 5. Вставляем в шаблон
        return this.template
            .replace('; >>> ZAKINIT WILL BE HERE <<<', zakinitString)
    ...
    reset() {
        ...
        this.modules.clear();
        this._activeCables = [];
        this._audioBusCount = 2;
        this._controlBusCount = 2;
        this.log('🔄 Reset');
        this.showFullCsd();
    }
}