...
export class LayerManager {
    ...
    updateCanvasSize() {
        ...
        console.log(`Layers: x=${this.layers.voice.x}, width=${this.layers.voice.width}`);
    }
    ...
    isGridCellFree(layerName, gridX, gridY, gridWidth = 1, gridHeight = 1, excludeModule = null) {
        ...
        const maxGridY = Math.floor(layer.visibleHeight / GRID_UNITS.Y) - gridHeight;
    ...
    handleWheel(deltaY, y) {
        if (y < this.divider.y) {
            ...
            this.layers.voice.scrollY = Math.max(0, Math.min(
            ...
            this.layers.fx.scrollY = Math.max(0, Math.min(
