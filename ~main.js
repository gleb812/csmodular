...
window.addEventListener('error', (event) => {
    ...
    if (modularSystem) {
        modularSystem.showNotification(`💥 Error: ${event.error?.message || 'Unknown error'}`);
    }
...
window.addEventListener('unhandledrejection', (event) => {
    ...
    if (modularSystem) {
        modularSystem.showNotification(`💥 Promise rejected: ${event.reason?.message || 'Unknown'}`);
    }
});
...
class ModularSystem {
    ...
    endMeasure(name) {
        ...
        // Сглаживание
        const alpha = 0.3;
        this.profiler.times[name] = this.profiler.times[name] * (1 - alpha) + duration * alpha;
    ...
    toggleProfiler(enabled) {
        if (!this.profiler) return;
        this.profiler.enabled = enabled !== undefined ? enabled : !this.profiler.enabled;
    ...
    setupCanvas() {
        console.log('=== SETUP CANVAS (FIXED SCALE, LEFT-ALIGNED) ===');
    ...
    async createPlaceholderModule(moduleName, moduleId, hpos, vpos, layerName) {
        ...
        // Создаем заглушку
        const placeholder = this.moduleFactory.createModule(placeholderName, hpos, vpos, layerName);
    ...
    collectCablesForExport() {
        ...
        return this.patchManager.cables.map(cable => {
            ...
            const fromPin = cable.fromJack.constructor?.name === 'Output' ? 'out' : 'in';
    ...
    savePatch(filename = null) {
        try {
            ...
            const actualFilename = filename || this.currentPatchFilename || 'patch_export.json';
    ...
    savePatchAs() {
        ...
        const filename = prompt('Enter filename for patch:', 
    ...
    async loadJsonPatch() {
        console.log('🔄 main.js: loadJsonPatch() called');
        
        try {
            console.log('   Calling patchLoader.loadPatchFromFile()...');
            ...
            console.log('   loadPatchFromFile result:', result);
            ...
            if (result.success) {
                ...
                const moduleCount = this.components.filter(c => c instanceof Panel).length;
                const cableCount = this.patchManager.cables.length;
                
                console.log(`   Load successful: ${moduleCount} modules, ${cableCount} cables`);
                
                if (this.uiManager && this.uiManager.updatePatchInfo) {
                    this.uiManager.updatePatchInfo(result.filename, moduleCount, cableCount);
                }
    ...
    async addNewModule(moduleType, layerName, gridX = null, gridY = null) {
        ...
        if (freeSpace) {
            return this.addNewModuleAtPosition(moduleType, layerName, freeSpace.gridX, freeSpace.gridY);
        } else {
            this.showNotification(`❌ Нет места в слое ${layerName === 'voice' ? 'VA' : 'FX'}`);
            return null;
        }
    }
    ...
    removeModule(module) {
        ...
        const cablesToRemove = this.patchManager.findCablesByModule(module.moduleId);
    ...
    isGridCellFree(layerName, gridX, gridY, gridWidth = 1, gridHeight = 1, excludeModule = null) {
        return this.layerManager.isGridCellFree(layerName, gridX, gridY, gridWidth, gridHeight, excludeModule);
    }

    findFreeSpace(layerName, gridWidth = 1, gridHeight = 1, startGridX = null, startGridY = null) {
        return this.layerManager.findFreeSpace(layerName, gridWidth, gridHeight, startGridX, startGridY);
    }
    ...
    animate() {
        ...
        const needsRedraw = this.draggingComponent || 
    ...
    async addNewModuleAtPosition(moduleType, layerName, gridX, gridY) {
        console.log(`=== ADD NEW MODULE AT POSITION: ${moduleType} to ${layerName} at (${gridX}, ${gridY}) ===`);
        ...
        try {
            ...
            if (!this.isGridCellFree(layerName, gridX, gridY, gridWidth, gridHeight)) {
                ...
                const freeSpace = this.findFreeSpace(layerName, gridWidth, gridHeight, gridX, gridY);
                if (!freeSpace) {
                    this.showNotification(`❌ Нет места в слое ${layerName === 'voice' ? 'VA' : 'FX'}`);
                    return;
                }
            ...
            // Создаем модуль
            const newModule = this.moduleFactory.createModule(moduleType, gridX, gridY, layerName);
            ...
            if (newModule) {
                ...
                const defaultMode = moduleDef?.mode || [];  // 👈 ИСПРАВЛЕНО: mode, не defaultModes
                
                console.log(`🎯 Adding module to csoundGen:`, {
                    typeId: moduleType,
                    instanceId: newModule.jsonId,
                    instanceName: newModule.title,
                    layer: layerName,
                    defaultParams: defaultParams,
                    defaultMode: defaultMode,                // 👈 ИСПРАВЛЕНО
                ...
                this.sortComponentsByZIndex();
                this.showNotification(`✅ ${moduleDef.displayName || moduleType} добавлен в (${gridX}, ${gridY})`);
    ...
    drawDraggingCableIfNeeded() {
        if (this.patchManager.draggingCable && this.patchManager.draggingCable.isDragging) {
            // Рисуем ПРЯМО на canvas без трансформаций
            this.patchManager.drawDraggingCable(this.ctx, this.offsetX, this.offsetY, this.scale);
        }
    }
    ...
    drawLayer(layerName) {
        ...
        if (cache) {
            this.ctx.drawImage(
                cache,
                layer.x, layer.y, // позиция на canvas
                cache.width, cache.height // рисуем полностью
            );
        }

        // СЕТКА - используем кеш
        if (this.frameCount % 3 === 0) { // Каждые 3 кадра
            const gridCache = this.layerManager.getGridCache(layerName, this.offsetX, this.offsetY, this.scale);
            if (gridCache) {
        ...
        if (this.lastMouseX !== null && this.lastMouseY !== null) {
            ...
            if (worldX >= layer.x && worldX <= layer.x + layer.width &&
                worldY >= layer.y && worldY <= layer.y + layer.visibleHeight) {
        ...
        if (this.draggingComponent && 
            ...
            if (worldX >= layer.x && worldX <= layer.x + layer.width &&
                ...
                this.ctx.fillRect(pixelX, pixelY, 
                ...
                this.ctx.strokeRect(pixelX, pixelY, 
                    module.gridWidth * GRID_UNITS.X, 
                    module.gridHeight * GRID_UNITS.Y);
                this.ctx.setLineDash([]);
                ...
                this.ctx.lineWidth = 3;
                this.ctx.strokeRect(currentPixelX, currentPixelY, 
                    module.gridWidth * GRID_UNITS.X, 
                    module.gridHeight * GRID_UNITS.Y);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.debugState = () => {
        ...
        if (modularSystem.layerManager) {
            Object.entries(modularSystem.layerManager.layers).forEach(([name, layer]) => {
        ...
        if (modularSystem.patchLoader?.moduleMap) {
            ...
            console.log(`Total in byId: ${Object.keys(modularSystem.patchLoader.moduleMap.byId || {}).length}`);
            console.log(`Voice layer: ${(modularSystem.patchLoader.moduleMap.byLayer?.voice || []).length}`);
            console.log(`FX layer: ${(modularSystem.patchLoader.moduleMap.byLayer?.fx || []).length}`);
            
            // Детали
            console.log('\n=== DETAILS ===');
            Object.entries(modularSystem.patchLoader.moduleMap.byId || {}).forEach(([id, module]) => {
        ...
        console.log('\n=== INTEGRITY CHECK ===');
        const panels = modularSystem.components.filter(c => c.constructor.name === 'Panel');
        console.log(`Panels in components: ${panels.length}`);
        
        panels.forEach((panel, i) => {
            console.log(`  [${i}] ${panel.title} (jsonId: ${panel.jsonId}, layer: ${panel.layer})`);
        });
    };
    ...
    window.debugModules = () => {
        ...
        modularSystem.components.forEach((comp, i) => {
            if (comp.constructor.name === 'Panel') {
                console.log(`[${i}] ${comp.title} (id: ${comp.moduleId}, jsonId: ${comp.jsonId})`);
            }
        });
    };
    ...
    console.log('ModularSystem initialized. Type debugSystem() for debug commands');
});